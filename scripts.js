document.addEventListener('DOMContentLoaded', () => {
    ...
    function initAllScripts() {
        ...
        // --- 2. RELEASES (UPDATED FOR BUNDLE MODAL) ---
        safeRun(() => {
            ...
            if (releasesContainer) {
                ...
                // FETCH BUNDLE INFO FOR RELEASES MODAL
            ...
            if(merchGrid || comingSoon) {
                fetch('store.json').then(r => r.json()).then(settings => {
                    ...
                    if (settings.merchActive) {
                        // STORE IS ON -> HIDE COMING SOON, SHOW GRID
                        ...
                        fetch('merch.json?t=' + new Date().getTime()).then(r => r.json()).then(data => {
                            ...
                            if(products.length === 0) {
                                ...
                                products.forEach(prod => {
                                    merchGrid.innerHTML += `
                                    <div class="gallery-item" style="aspect-ratio:auto;">
                                        ...
                                        <div style="padding:1rem; text-align:center; background:rgba(0,0,0,0.5);">
                                            ...
                                            <a href="${prod.link}" target="_blank" class="btn btn-outline" style="width:100%;">BUY NOW</a>
                                        </div>
                                    </div>`;
                                });
                            }
                        ...
                        // STORE IS OFF -> SHOW COMING SOON, HIDE GRID
        ...
        safeRun(() => {
            ...
            if (beatCont) {
                ...
                // FETCH SETTINGS FOR INFO MODAL (MOBILE)
                ...
                if(vBtn) {
                    vBtn.onclick = () => {
                        const modal = document.getElementById('vibe-modal'); modal.classList.add('visible'); const bubbles = document.getElementById('vibe-bubbles-container');
                        let waveform = modal.querySelector('.modal-box .waveform-container'); if(!waveform) { const box = modal.querySelector('.modal-box'); waveform = document.createElement('div'); waveform.className = 'waveform-container'; for(let i=0; i<30; i++) { const bar = document.createElement('div'); bar.className = 'waveform-bar'; bar.style.animationDelay = `${Math.random()}s`; waveform.appendChild(bar); } box.appendChild(waveform); }
                        if(bubbles.innerHTML === '') { fetch('vibes.json').then(r=>r.json()).then(d => { (d.vibes||[]).forEach(v => { const b = document.createElement('button'); b.className='floating-vibe'; b.textContent=v.name; b.style.animationDelay = `${Math.random() * 2}s`; b.onmouseenter = () => { const color = v.color || '#8a2be2'; b.style.color = color; b.style.borderColor = color; b.style.boxShadow = `0 0 15px ${color}`; document.querySelectorAll('.waveform-bar').forEach(bar => { bar.style.background = color; bar.style.boxShadow = `0 0 10px ${color}`; }); const modalBox = document.querySelector('.modal-box'); if(modalBox) { modalBox.style.borderColor = color; modalBox.style.boxShadow = `0 0 40px ${color}40`; } }; b.onmouseleave = () => { b.style.color = '#fff'; b.style.borderColor = 'rgba(255,255,255,0.1)'; b.style.boxShadow = 'none'; document.querySelectorAll('.waveform-bar').forEach(bar => { bar.style.background = '#8a2be2'; bar.style.boxShadow = 'none'; }); const modalBox = document.querySelector('.modal-box'); if(modalBox) { modalBox.style.borderColor = 'rgba(138, 43, 226, 0.5)'; modalBox.style.boxShadow = '0 0 30px rgba(138, 43, 226, 0.2)'; } }; b.onclick = () => { document.getElementById('vibe-modal').classList.remove('visible'); const f = window.currentPlaylist.filter(beat => beat.tags && beat.tags.some(t => v.tags.includes(t))); renderBeats(f); }; bubbles.appendChild(b); }); }); }
                    };
        ...
        // --- 5. BIO, GALLERY, MENU, FOOTER ---
        safeRun(() => {
            const bioContainer = document.getElementById('bio-container');
            if (bioContainer) { fetch('bio.json').then(r => r.ok ? r.json() : Promise.reject()).then(data => { const content = data.content ? data.content.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\n/g, '<br>') : '...'; if(data.title && document.getElementById('bio-title')) document.getElementById('bio-title').textContent = data.title; bioContainer.innerHTML = `<div class="bio-image-wrapper"><img src="${data.image}" class="bio-img"></div><div class="bio-text"><p>${content}</p></div>`; }).catch(() => {}); }
            const galleryGrid = document.getElementById('gallery-grid');
            if(galleryGrid && !document.getElementById('merch-grid')) { // Avoid conflict with store grid
                const gModal = document.getElementById('gallery-modal'); const gImg = document.getElementById('gallery-modal-img'); const gCap = document.getElementById('gallery-caption'); const gClose = document.getElementById('close-gallery-modal');
                fetch('gallery.json').then(r => r.json()).then(data => { galleryGrid.innerHTML = ''; (data.images || []).forEach(img => { const div = document.createElement('div'); div.className = 'gallery-item'; div.innerHTML = `<img src="${img.src}" alt="${img.caption || ''}">`; div.onclick = () => { gImg.src = img.src; gCap.innerText = img.caption || ''; gModal.classList.add('visible'); }; galleryGrid.appendChild(div); }); }).catch(() => {});
