document.addEventListener('DOMContentLoaded', () => {
    ...
    function initAllScripts() {
        ...
        if (hamburger) {
            ...
            newHamburger.addEventListener('click', () => { newHamburger.classList.toggle('active'); navLinks.classList.toggle('active'); });
            document.querySelectorAll('.nav-btn').forEach(link => { link.addEventListener('click', () => { newHamburger.classList.remove('active'); navLinks.classList.remove('active'); }); });
        }
        ...
        if (menuContainer && menuContainer.innerHTML === '') {
            fetch('menu.json').then(r => r.json()).then(data => {
                ...
                links.forEach(link => { const target = link.newTab ? '_blank' : '_self'; menuHtml += `<a href="${link.url}" class="nav-btn" target="${target}">${link.text}</a>`; });
        ...
        if(playBtn) {
            ...
            window.playTrack = function(url, title, cover, trackIndexInList) {
                ...
                window.currentIndex = trackIndexInList; audio.src = url; if(playerTitle) playerTitle.textContent = title;
                if (cover) { window.currentCover = cover; restoreHeroArt(); } else { window.currentCover = 'https://via.placeholder.com/600/111/333?text=V'; restoreHeroArt(); }
            ...
            playBtn.onclick = togglePlay; if(nextBtn) nextBtn.onclick = playNext; if(prevBtn) prevBtn.onclick = playPrev;
            ...
            audio.onended = playNext; audio.onplay = () => { window.isPlaying = true; updateUIState(); checkPlayerVisibility(); }; audio.onpause = () => { window.isPlaying = false; updateUIState(); };
        }

        // --- 7. BEATS LOADER ---
        const beatContainer = document.getElementById('beat-store-list');
        if (beatContainer) {
            const filterGenre = document.getElementById('filter-genre'); const filterBpm = document.getElementById('filter-bpm'); const filterKey = document.getElementById('filter-key');
            let allBeats = [];
            ...
            const vBtn = document.getElementById('vibe-search-btn'); const vModal = document.getElementById('vibe-modal'); const vClose = document.getElementById('vibe-modal-close'); const vBubbles = document.getElementById('vibe-bubbles-container');
            if (vBtn && vModal) {
                vBtn.onclick = () => { vModal.classList.add('visible'); if (vBubbles.innerHTML === '') { fetch('vibes.json').then(r => r.json()).then(data => { let vibes = Array.isArray(data) ? data : (data.vibes || []); vibes.forEach(vibe => { const b = document.createElement('button'); b.className = 'btn floating-vibe'; b.textContent = vibe.name; b.onclick = () => { vModal.classList.remove('visible'); const f = allBeats.filter(beat => { if(!beat.tags) return false; return beat.tags.some(t => vibe.tags.includes(t)); }); window.currentPlaylist = f; renderBeats(f); }; vBubbles.appendChild(b); }); }); } }; 
                vClose.onclick = () => vModal.classList.remove('visible'); vModal.onclick = (e) => { if(e.target === vModal) vModal.classList.remove('visible'); };
            }
        }
        ...
        // --- RELEASES (FIXED WITH ERROR LOGGING) ---
        ...
        if (releasesList) {
            fetch('releases.json')
                ...
                .then(data => {
                    ...
                    if (tracks.length === 0) { releasesList.innerHTML = '<p style="text-align:center;">No releases found.</p>'; return; }
                    ...
                    tracks.forEach((track) => { 
                        ...
                        const safeTitle = (track.title || 'Unknown Track').replace(/'/g, "\\'");
                ...
                .catch(err => {
                    ...
                    releasesList.innerHTML = `<p style="color:red; text-align:center;">Error loading tracks: ${err.message}</p>`;
                });
        }
    ...
    function updateMenuState() {
        ...
        document.querySelectorAll('.nav-btn').forEach(link => {
            ...
            if (linkPath === currentPath || (currentPath === '' && linkPath === 'index.html')) { link.classList.add('active'); } else { link.classList.remove('active'); }
        });
    }
    ...
    function checkPlayerVisibility() {
        const stickyPlayer = document.getElementById('sticky-player'); if (!stickyPlayer) return;
        ...
        if (isBeatsPage || hasActiveTrack || window.isPlaying) { stickyPlayer.classList.add('player-visible'); } else { stickyPlayer.classList.remove('player-visible'); }
    }
    ...
    function setupCustomDropdowns(allBeats) {
        ...
        dropdowns.forEach(dropdown => {
            const btn = dropdown.querySelector('.select-btn'); const list = dropdown.querySelector('.select-options'); const span = btn.querySelector('span');
    ...
    function renderBeats(beats) { 
        const beatContainer = document.getElementById('beat-store-list'); if (!beatContainer) return; beatContainer.innerHTML = ''; 
        ...
        beats.forEach((beat, index) => { 
            const safeTitle = beat.title.replace(/'/g, "\\'"); const beatImage = beat.cover || 'https://via.placeholder.com/600/111/333?text=V'; 
