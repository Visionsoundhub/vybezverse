document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (e) => {
        ...
        // --- 5. PAYHIP DELEGATION (HERE IS THE FIX) ---
        const payhipBtn = e.target.closest('.payhip-buy-btn');
        if (payhipBtn) {
            e.preventDefault(); // Stop default navigation
            const url = payhipBtn.getAttribute('data-href');
            
            // Check if Payhip is loaded
            if (typeof Payhip !== 'undefined') {
                Payhip.Checkout.open(url);
            } else {
    ...
    // --- UPDATED RENDER BEATS (Clean HTML, no inline onclick) ---
    function renderBeats(beats) { 
        ...
        beats.forEach((b, i) => { 
            ...
            const payhipClass = isPayhip ? 'payhip-buy-btn' : ''; 
            ...
            <div class="beat-row" id="beat-row-${slug}">
                ...
                <div class="beat-actions">
                    ...
                    <a href="${b.checkoutUrl}" class="btn btn-accent ${payhipClass}" data-href="${b.checkoutUrl}" target="_blank">${b.price} | BUY</a>
                </div>
            </div>`; 
        }); 
    }
